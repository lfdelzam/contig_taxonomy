...
files_arch=[f for f in os.listdir(os.path.join(args.d, "archaea")) if f.endswith(".faa")]
files_bact=[f for f in os.listdir(os.path.join(args.d, "bacteria")) if f.endswith(".faa")]

with open(args.o, "w") as fout:
    for file in files_arch:
        id=file.split("_protein")[0]
        if id in taxo_arch:
            with open(os.path.join(args.d,"archaea", file), "r") as fin:
                copy=False
                for line1 in fin:
                    line1=line1.rstrip()
                    if line1.startswith(">"):
...
#printing out last sequence
    ...
    for fileb in files_bact:
        ...
        if idb in taxo_bact:
            with open(os.path.join(args.d,"bacteria", fileb), "r") as fin2:
                copy=False
                for lineb in fin2:
                    lineb=lineb.rstrip()
                    if lineb.startswith(">"):
